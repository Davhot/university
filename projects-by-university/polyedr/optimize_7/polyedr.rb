...
class Edge 
  ...
  def shadow(facet)
    # "низкие" и вертикальные грани не могут затенять ребро
    if self.beg.z >= facet.zmax && self.fin.z >= facet.zmax || facet.vertical?
      return true
    end
    # xy-прямоугольник грани должен пересекать xy-прямоугольник ребра
    if [self.beg.x, self.fin.x].min > facet.xmax or
        [self.beg.x, self.fin.x].max < facet.xmin or 
        [self.beg.y, self.fin.y].min > facet.ymax or
        [self.beg.y, self.fin.y].max < facet.ymin
      return true
    end
...
# Грань полиэдра
class Facet 
  attr_accessor :xmin, :xmax, :ymin, :ymax, :zmax
  ...
  def precompile
    ...
    @xmin = @ymin = TkDrawer::SIZE/2
    @xmax = @ymax = @zmax = -TkDrawer::SIZE/2
    @vertexes.each do |v| 
      @xmin = v.x if v.x < @xmin; @ymin = v.y if v.y < @ymin
      @xmax = v.x if v.x > @xmax; @ymax = v.y if v.y > @ymax
      @zmax = v.z if v.z > @zmax
    end
  end
end
...
class Polyedr 
  ...
  def smart_shadow(e)
    # хэш учтённых граней
    processed = {}
    for i in to_range(e.beg.x, e.fin.x)
      for j in to_range(e.beg.y, e.fin.y)
        @nests[[i,j]].each do |f|
          if processed[f].nil?
            processed[f] = true
            # прекращаем обработку, если просветов на ребре не осталось
            return unless e.shadow(f)
          end
        end
      end
    end
  ...
  def facets_nests
    ...
    @step = (0...100).inject(0.0) do |s,i|
      e = @edges[rand(@edges.size)]
      s + Math.sqrt((e.fin.x-e.beg.x)**2+(e.fin.y-e.beg.y)**2)
    end / 200.0
    printf "     Размер гнёзд: %9.2f\n", @step
    @facets.each do |f|
      rx, ry = ranges(f)
      for i in rx
	for j in ry
	  key = [i,j]
	  @nests.include?(key) ? @nests[key].push(f) : @nests[key] = [f]
	end
      end
    end
  end

  # множество гнёзд для грани
  def ranges(f)
    [to_range(f.xmin, f.xmax), to_range(f.ymin, f.ymax)]
  end

  # диапазон индексов гнёзд для отрезка
  def to_range(t1, t2)
    t1,t2 = t2,t1 if t1 > t2
    ((t1/@step).floor .. (t2/@step).ceil)
  end
